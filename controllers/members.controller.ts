...
export const addMemberInWorkspace = async (req: Request, res: Response) => {
  try {
    ...
    if (!workspace)
      return res.status(404).send("No workspace found with the given ID.");

    if (workspace.ownerId !== user.id)
      return res.status(400).send("You don't have permission to add members.");

    const m = await prisma.workspaceMember.findFirst({
      where: {
        AND: [{ workspaceId }, { userId: memberId }],
      },
    });

    if (m)
      return res.status(400).send("This user is already in this workspace.");
...
export const getMembersByWorkspace = async (req: Request, res: Response) => {
  try {
    ...
    if (!project)
      return res.status(400).send("No item found with the given ID.");
